...
testOpenTodoFileWithExistingFile() {
  ...
  export TODO_DIR=$(mktemp -d)
  mkdir -p "$TODO_DIR/dss"
  touch "$TODO_DIR/dss/dss.md"
  result=$(open_todo_file "dss")
  # expect a project specific file
  assertEquals "$TODO_DIR/dss/dss.md" "$result"
  rm -r "$TODO_DIR"

  export TODO_DIR=$(mktemp -d)
  mkdir -p "$TODO_DIR/dss"
  touch "$TODO_DIR/dss/todo.md"
  result=$(open_todo_file "dss")
  # expect a project specific file
  assertEquals "$TODO_DIR/dss/todo.md" "$result"
  rm -r "$TODO_DIR"
}
